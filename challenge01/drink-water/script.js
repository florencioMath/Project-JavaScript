function drinkWater() {
  ...
  cupSmall.forEach((cup, index) => {
    cup.addEventListener("click", () => drink(index));
  });
  ...
  function fullCup() {
    const totalSmallCupsFull =
      document.querySelectorAll(".cupSmall.full").length;
    ...
    if (totalSmallCupsFull === 0) {
      ...
      percentage.style.height = `${(totalSmallCupsFull / totalCups) * 350}px`;
